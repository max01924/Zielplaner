import { CalendarPlus, Check, ChevronDown, Link2, Pencil, Plus, Save, Trash2, X } from "lucide-react";
      ...
      <div className="flex items-start gap-3">
        <button
          ...
          className={`grid h-6 w-6 shrink-0 self-center place-items-center rounded-lg transition ${
            priority.done ? "bg-accent text-ink" : "bg-canvas-deep text-transparent hover:bg-surface-hover"
          }`}
          ...
          {isEditing ? (
            <div className="space-y-3">
              <input
                ...
                autoFocus
              />
              <ParentSelect value={draftParentId} onChange={setDraftParentId} options={parentOptions} label="Monatsziel verknüpfen" />
            </div>
          ) : (
            <>
...
export default function WeeklyPriorities({ priorities, tasks, monthlyGoals, parentPrefill, onNavigateParent, onNavigateTask, onDeriveTask, onCreate, onUpdate, onDelete }) {
  ...
  return (
    <div className="space-y-10">
      <form onSubmit={submit} className="bg-depth-panel rounded-panel p-5 shadow-card sm:p-6">
        ...
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(260px,0.8fr)_auto] lg:items-end">
          <label>
          ...
          <ParentSelect value={monthlyGoalId} onChange={setMonthlyGoalId} options={monthlyGoals} label="Monatsziel verknüpfen" disabled={!canAdd} />
          <button
            type="submit"
            disabled={!canAdd || !text.trim()}
            className="inline-flex min-h-12 items-center justify-center gap-2 rounded-control bg-accent px-5 text-sm font-black text-ink shadow-inset transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-40"
          >
